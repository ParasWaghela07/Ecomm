    ...
    <div className="min-h-screen bg-gray-100 p-8">
      {user ? (
        <div className="max-w-md mx-auto bg-white rounded-lg shadow-md p-6">
         ...
         <button onClick={() => setcart(prev => [...prev, { id: 123, name: "hello" }])}>Counter</button>
            ...
            {products?.map((shoe,index)=>{
              return (
                <div key={index} className="mt-4 p-4 bg-gray-50 rounded-md overflow-y-auto">
                  ...
                  <button className='px-4 py-2 bg-blue-500 rounded-xl text-white font-bold cursor-pointer' onClick={()=>{addtocart(shoe)}}>Add to cart</button>
                </div>
              );
            })}
